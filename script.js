...
function renderSummaryTable() {
  ...
  data.veicoli.forEach(v => {
    ...
    // Pulsante elimina veicolo
    const btnDelete = document.createElement("button");
    btnDelete.className = "btn btn-danger";
    btnDelete.textContent = "🗑️ Elimina";
    btnDelete.onclick = () => deleteVehicle(v.id, v.name);
...
function deleteVehicle(id, vehicleName){
  // Controlla se ci sono incassi associati a questo veicolo
  const hasIncomes = data.incassi.some(i => i.vehicleId === id);
  
  let confirmMsg = `Sei sicuro di voler eliminare il veicolo "${vehicleName}"?`;
  if(hasIncomes){
    confirmMsg += '\n\n⚠️ ATTENZIONE: Questo veicolo ha degli incassi registrati. Eliminando il veicolo, verranno eliminati anche tutti i suoi incassi!';
  }
  
  if(!confirm(confirmMsg)) return;
  
  // Rimuovi tutti gli incassi del veicolo
  data.incassi = data.incassi.filter(i => i.vehicleId !== id);
  
  // Rimuovi il veicolo
  const idx = data.veicoli.findIndex(v => v.id === id);
  if(idx !== -1){
    data.veicoli.splice(idx, 1);
  }
  
  // Se sto modificando un incasso di questo veicolo, esci dalla modalità edit
  if(editingIncomeId){
    const editingIncome = data.incassi.find(i => i.id === editingIncomeId);
    if(!editingIncome) exitEditMode();
  }
  
  saveData();
  refreshUI();
  
  alert(`✅ Veicolo "${vehicleName}" eliminato con successo${hasIncomes ? ' (insieme ai suoi incassi)' : ''}.`);
}
